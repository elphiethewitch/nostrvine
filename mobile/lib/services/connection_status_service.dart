...
class ConnectionStatusService extends ChangeNotifier {
  ...
  List<ConnectivityResult> get connectionTypes =>
      List.unmodifiable(_connectionTypes);
  ...
  String get connectionStatus {
    ...
    if (_connectionTypes.contains(ConnectivityResult.mobile)) {
      return 'Mobile Data';
    }
  ...
  Future<void> initialize() async {
    try {
      ...
      Log.error('Failed to initialize connection status service: $e',
          name: 'ConnectionStatusService', category: LogCategory.system);
  ...
  Future<void> _checkConnectivity() async {
    try {
      ...
      Log.debug(
          '📱 Initial connectivity: $_connectionTypes, online: $_isOnline, internet: $_hasInternetAccess',
  ...
  Future<void> _checkInternetAccess() async {
    ...
    try {
      ...
      if (kIsWeb) {
        ...
        hasAccess = _isOnline;
        Log.debug(
            '📱 Web platform: Using connectivity assumption for internet access',
            name: 'ConnectionStatusService',
            category: LogCategory.system);
      } else {
        ...
        for (final host in hosts) {
          try {
            ...
            Log.error('Failed to reach $host: $e',
                name: 'ConnectionStatusService', category: LogCategory.system);
          }
        }
      }
      ...
      if (hasAccess && !hadAccess) {
        Log.debug('📱 Internet access restored',
            name: 'ConnectionStatusService', category: LogCategory.system);
      } else if (!hasAccess && hadAccess) {
        Log.warning('Internet access lost',
            name: 'ConnectionStatusService', category: LogCategory.system);
      }
  ...
  Future<void> forceCheck() async {
    Log.debug('Force checking connection status...',
        name: 'ConnectionStatusService', category: LogCategory.system);
    await _checkConnectivity();
  }
  ...
  Future<bool> waitForConnection(
    ...
    Log.debug('⏳ Waiting for connection to be restored...',
        name: 'ConnectionStatusService', category: LogCategory.system);
