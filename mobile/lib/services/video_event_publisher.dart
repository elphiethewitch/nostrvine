...
class VideoEventPublisher  {
  ...
  Future<void> startPolling() async {
    if (_isPollingActive) {
      Log.warning('Polling already active',
          name: 'VideoEventPublisher', category: LogCategory.video);
      return;
    }
    ...
    Log.debug(
        'Starting video event polling (interval: ${_currentPollInterval.inSeconds}s)',
    ...
    try {
      ...
      _pollTimer =
          Timer.periodic(_currentPollInterval, (_) => _checkForReadyEvents());
    } catch (e) {
      Log.warning(
          'Polling endpoint not available - using direct upload only mode',
          name: 'VideoEventPublisher',
          category: LogCategory.video);
  ...
  void stopPolling() {
    ...
    Log.debug('Stopping video event polling',
        name: 'VideoEventPublisher', category: LogCategory.video);
  ...
  Future<void> _checkForReadyEvents() async {
    if (!_isPollingActive || !_isAppActive) {
      Log.warning('Skipping poll - app inactive or polling stopped',
          name: 'VideoEventPublisher', category: LogCategory.video);
      return;
    }

    try {
      Log.debug('Checking for ready events...',
          name: 'VideoEventPublisher', category: LogCategory.video);
      ...
      if (readyEvents.isEmpty) {
        Log.info('No ready events found',
            name: 'VideoEventPublisher', category: LogCategory.video);
        _updatePollInterval();
        return;
      }

      Log.info('Found ${readyEvents.length} ready events',
          name: 'VideoEventPublisher', category: LogCategory.video);
      ...
      Log.error('Error checking ready events: $e',
          name: 'VideoEventPublisher', category: LogCategory.video);
      ...
      if (e.toString().contains('network') ||
        ...
        Log.error('📱 Network error detected, will retry later',
            name: 'VideoEventPublisher', category: LogCategory.video);
        _scheduleRetry();
      }
    }
  ...
  Future<void> _processReadyEvent(ReadyEventData eventData) async {
    try {
      Log.debug('Processing ready event: ${eventData.publicId}',
          name: 'VideoEventPublisher', category: LogCategory.video);

      if (!eventData.isReadyForPublishing) {
        Log.warning(
            'Event not ready for publishing: ${eventData.statusDescription}',
            name: 'VideoEventPublisher',
            category: LogCategory.video);
        return;
      }
      ...
      if (publishResult) {
        ...
        Log.info('Successfully published event: ${nostrEvent.id}',
            name: 'VideoEventPublisher', category: LogCategory.video);
      } else {
        throw Exception('Failed to publish to Nostr relays');
      }
    } catch (e, stackTrace) {
      Log.error('Failed to process event ${eventData.publicId}: $e',
          name: 'VideoEventPublisher', category: LogCategory.video);
      Log.verbose('📱 Stack trace: $stackTrace',
          name: 'VideoEventPublisher', category: LogCategory.video);
  ...
  Future<Event?> _createVideoEvent(ReadyEventData eventData) async {
    if (_authService == null || !_authService!.isAuthenticated) {
      Log.error('Cannot create video event - user not authenticated',
          name: 'VideoEventPublisher', category: LogCategory.video);
      return null;
    }
    ...
    try {
      ...
      // Create Kind 22 tags per NIP-71 specification
      final videoTags = <List<String>>[];
      // NIP-71: Use 'r' tags for resources with type specification
      if (videoUrl != null) videoTags.add(['r', videoUrl, 'video']); // NIP-71 compliant
      if (thumbnailUrl != null) videoTags.add(['r', thumbnailUrl, 'thumbnail']); // NIP-71 compliant
      
      // Standard NIP-71 metadata tags
      ...
      if (event == null) {
        Log.error('Failed to create and sign video event',
            name: 'VideoEventPublisher', category: LogCategory.video);
        return null;
      }

      Log.info('📱 Created Kind 22 video event: ${event.id}',
          name: 'VideoEventPublisher', category: LogCategory.video);
      Log.debug('Video URL: $videoUrl',
          name: 'VideoEventPublisher', category: LogCategory.video);
      Log.debug('Event size: ${eventData.estimatedEventSize} bytes',
          name: 'VideoEventPublisher', category: LogCategory.video);
      Log.verbose('Tags: ${event.tags.length}',
          name: 'VideoEventPublisher', category: LogCategory.video);

      return event;
    } catch (e) {
      Log.error('Error creating video event: $e',
          name: 'VideoEventPublisher', category: LogCategory.video);
      return null;
    }
  }
  ...
  Future<bool> _publishEventToNostr(Event event) async {
    try {
      ...
      Log.info('Event published successfully to relays',
          name: 'VideoEventPublisher', category: LogCategory.video);
      return true;
    } catch (e) {
  ...
  Future<void> _updateLocalUploadStatus(
      ReadyEventData eventData, String nostrEventId) async {
    ...
    if (upload != null) {
      ...
      Log.debug(
          '📱 Updated local upload status: ${eventData.originalUploadId} -> published',
          name: 'VideoEventPublisher',
          category: LogCategory.video);
      Log.debug('📱 Linked to Nostr event: $nostrEventId',
          name: 'VideoEventPublisher', category: LogCategory.video);
    } else {
      Log.warning(
          'Could not find local upload for: ${eventData.originalUploadId}',
          name: 'VideoEventPublisher',
          category: LogCategory.video);
    }
  }
  ...
  Future<void> _showPublishNotification(
      ReadyEventData eventData, String nostrEventId) async {
    try {
      // Show platform notification
      // TODO: Implement actual notification service
      Log.debug('📱 Would show notification: Video published!',
          name: 'VideoEventPublisher', category: LogCategory.video);
      Log.debug('📱 Event ID: $nostrEventId',
          name: 'VideoEventPublisher', category: LogCategory.video);
      Log.debug('Video URL: ${eventData.secureUrl}',
          name: 'VideoEventPublisher', category: LogCategory.video);
    } catch (e) {
  ...
  void _updatePollInterval() {
    final hasPendingUploads =
        _uploadManager.getUploadsByStatus(UploadStatus.processing).isNotEmpty ||
            _uploadManager
                .getUploadsByStatus(UploadStatus.readyToPublish)
                .isNotEmpty;
    ...
    if (_isPollingActive && _pollTimer != null) {
      ...
      _pollTimer =
          Timer.periodic(_currentPollInterval, (_) => _checkForReadyEvents());
      Log.debug('Updated poll interval to ${_currentPollInterval.inSeconds}s',
          name: 'VideoEventPublisher', category: LogCategory.video);
    }
  }
  ...
  void _setupAppLifecycleListener() {
    SystemChannels.lifecycle.setMessageHandler((message) async {
      Log.debug('📱 App lifecycle: $message',
          name: 'VideoEventPublisher', category: LogCategory.video);
        ...
        case 'AppLifecycleState.resumed':
          ...
          if (_lastAppBackgroundTime != null) {
            ...
            if (backgroundDuration.inMinutes > 10) {
              Log.debug(
                  'App resumed after ${backgroundDuration.inMinutes}min, checking immediately',
                  name: 'VideoEventPublisher',
                  category: LogCategory.video);
  ...
  void _scheduleRetry() {
    ...
    _retryTimer = Timer(_retryDelay, () {
      Log.error('Retrying after network error',
          name: 'VideoEventPublisher', category: LogCategory.video);
      _checkForReadyEvents();
    });
  }
  ...
  void _scheduleRetryForFailedEvents() {
    ...
    _retryTimer = Timer(_retryDelay, () async {
      Log.error('Retrying ${_failedEvents.length} failed events',
          name: 'VideoEventPublisher', category: LogCategory.video);
  ...
  Future<void> forceCheck() async {
    Log.debug('Force checking for ready events',
        name: 'VideoEventPublisher', category: LogCategory.video);
    await _checkForReadyEvents();
  }
  ...
  Future<bool> publishDirectUpload(PendingUpload upload,
    ...
    try {
      ...
      // NIP-71: Use 'r' tags for resources with type specification
      tags.add(['r', upload.cdnUrl!, 'video']); // NIP-71 compliant resource tag
      tags.add(['m', 'video/mp4']); // MIME type

      // Add thumbnail if available (NIP-71 compliant)
