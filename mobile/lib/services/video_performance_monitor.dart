...
class VideoPerformanceMonitor extends ChangeNotifier {
  ...
  PerformanceSample? get currentSample =>
    ...
    if (duration > _thresholds.slowPreloadThreshold) {
      _triggerAlert(
        PerformanceAlert(
          ...
          message:
              'Slow preload detected: ${duration.inMilliseconds}ms for $videoId',
      ...
      if (failureRate > _thresholds.preloadFailureRateThreshold) {
        _triggerAlert(
          PerformanceAlert(
            ...
            message:
                'High preload failure rate: ${(failureRate * 100).toStringAsFixed(1)}%',
            timestamp: DateTime.now(),
            metadata: {
  ...
  void recordMemoryPressure(int memoryUsageMB, int controllersDisposed) {
    _triggerAlert(
      PerformanceAlert(
        ...
        message:
            'Memory pressure event: ${memoryUsageMB}MB, disposed $controllersDisposed controllers',
        timestamp: DateTime.now(),
        metadata: {
  ...
  void _checkThresholds(PerformanceSample sample) {
    ...
    // High failure rate
    final failureRate =
        sample.totalVideos > 0 ? sample.failedVideos / sample.totalVideos : 0.0;
  ...
  MemoryTrend _calculateMemoryTrend(List<PerformanceSample> samples) {
    if (samples.length < 2) {
      return const MemoryTrend(direction: TrendDirection.stable, changeRate: 0);
    }

    final recent = samples.skip(samples.length - 10).toList();
    final older = samples
        .take(samples.length - 10)
        .skip((samples.length - 20).clamp(0, samples.length - 10))
        .toList();

    final recentAvg =
        recent.fold(0, (sum, s) => sum + s.memoryUsageMB) / recent.length;
    final olderAvg =
        older.fold(0, (sum, s) => sum + s.memoryUsageMB) / older.length;
  ...
  PerformanceTrend _calculatePerformanceTrend(List<PerformanceSample> samples) {
    if (samples.length < 2) {
      return const PerformanceTrend(direction: TrendDirection.stable, score: 1);
    }

    // Calculate performance score based on multiple factors
    final recentSamples = samples.takeLast(10).toList();
    final avgMemory =
        recentSamples.fold(0.0, (sum, s) => sum + s.memoryUsageMB) /
            recentSamples.length;
    final avgControllers =
        recentSamples.fold(0.0, (sum, s) => sum + s.controllerCount) /
            recentSamples.length;
    final avgFailureRate = recentSamples.fold(
          0.0,
          (sum, s) =>
              sum + (s.totalVideos > 0 ? s.failedVideos / s.totalVideos : 0.0),
        ) /
        recentSamples.length;

    // Normalize scores (0-1, higher is better)
    final memoryScore =
        (1000 - avgMemory) / 1000; // Assume 1000MB is worst case
    final controllerScore =
        (20 - avgControllers) / 20; // Assume 20 controllers is worst case
  ...
  MemoryUsageAnalysis _analyzeMemoryUsage(List<PerformanceSample> samples) {
    if (samples.isEmpty) {
      return const MemoryUsageAnalysis(
        average: 0,
        peak: 0,
        minimum: 0,
        trend: MemoryTrend(direction: TrendDirection.stable, changeRate: 0),
        distribution: [],
      );
    }
    ...
    final average =
        memoryValues.fold(0, (sum, val) => sum + val) / memoryValues.length;
  ...
  PreloadPerformanceAnalysis _analyzePreloadPerformance() {
    ...
    if (allTimes.isEmpty) {
      return const PreloadPerformanceAnalysis(
    ...
    final slowPreloads = allTimes
        .where((time) => time > _thresholds.slowPreloadThreshold)
        .length;
    final fastPreloads =
        allTimes.where((time) => time < const Duration(seconds: 1)).length;
  ...
  AlertSummary _analyzeAlerts(DateTime? cutoff) {
    ...
    final criticalCount = relevantAlerts
        .where((a) => a.severity == AlertSeverity.critical)
        .length;
    final warningCount =
        relevantAlerts.where((a) => a.severity == AlertSeverity.warning).length;
    final infoCount =
        relevantAlerts.where((a) => a.severity == AlertSeverity.info).length;
    ...
    if (stats.currentMemoryMB > _thresholds.highMemoryThreshold) {
      recommendations.add(
        PerformanceRecommendation(
          ...
          description:
              'Current memory usage is ${stats.currentMemoryMB}MB. Consider reducing preload range or implementing more aggressive cleanup.',
    ...
    if (stats.currentControllers > _thresholds.maxControllers) {
      recommendations.add(
        PerformanceRecommendation(
          ...
          description:
              'Currently managing ${stats.currentControllers} controllers. Consider reducing the maximum limit.',
    ...
    if (stats.preloadSuccessRate < 0.9) {
      recommendations.add(
        PerformanceRecommendation(
          ...
          description:
              'Preload success rate is ${(stats.preloadSuccessRate * 100).toStringAsFixed(1)}%. Check network conditions and URL validity.',
  ...
  List<MemoryDistributionBucket> _calculateMemoryDistribution(
      List<int> memoryValues) {
    final buckets = <MemoryDistributionBucket>[];
    const bucketSize = 100; // 100MB buckets
    final maxMemory = memoryValues.isEmpty ? 0 : memoryValues.reduce(max);

    for (var i = 0; i <= maxMemory; i += bucketSize) {
      final count =
          memoryValues.where((val) => val >= i && val < i + bucketSize).length;
      if (count > 0) {
        buckets.add(
          MemoryDistributionBucket(
            rangeStart: i,
