...
class GlobalVideoRegistry {
  ...
  void registerController(VideoPlayerController controller) {
    if (_activeControllers.add(controller)) {
      Log.debug(
        '📹 Registered video controller. Total: ${_activeControllers.length}',
        name: 'GlobalVideoRegistry',
  ...
  void unregisterController(VideoPlayerController controller) {
    if (_activeControllers.remove(controller)) {
      Log.debug(
        '📹 Unregistered video controller. Remaining: ${_activeControllers.length}',
        name: 'GlobalVideoRegistry',
        category: LogCategory.video,
      );
    }
  ...
  void pauseAllControllers() {
    Log.debug(
      '🎬 pauseAllControllers called with ${_activeControllers.length} controllers',
      name: 'GlobalVideoRegistry',
      category: LogCategory.video,
    );
      ...
      try {
        if (controller.value.isInitialized) {
          if (controller.value.isPlaying) {
            playingCount++;
            Log.debug(
              '  - Pausing controller (playing: ${controller.value.isPlaying}, position: ${controller.value.position})',
              name: 'GlobalVideoRegistry',
              category: LogCategory.video,
            );
            controller.pause();
    ...
    Log.info(
      '⏸️ Paused $pausedCount of $playingCount playing video(s) from ${_activeControllers.length} total',
      name: 'GlobalVideoRegistry',
      category: LogCategory.video,
    );
  }
  ...
  void pauseAllExcept(VideoPlayerController? exceptController) {
    ...
    Log.debug(
      '🎬 pauseAllExcept called with ${_activeControllers.length} controllers',
      name: 'GlobalVideoRegistry',
      category: LogCategory.video,
    );
      ...
      try {
        if (controller.value.isInitialized) {
          if (controller.value.isPlaying) {
            playingCount++;
            Log.debug(
              '  - Pausing controller (playing: ${controller.value.isPlaying})',
              name: 'GlobalVideoRegistry',
              category: LogCategory.video,
            );
            controller.pause();
    ...
    Log.info(
      '⏸️ Paused $pausedCount of $playingCount playing video(s), skipped $skippedCount',
      name: 'GlobalVideoRegistry',
      category: LogCategory.video,
    );
  }
  ...
  void cleanupDisposedControllers() {
    ...
    if (disposedControllers.isNotEmpty) {
      Log.debug(
        '🧹 Cleaned up ${disposedControllers.length} disposed controller(s)',
        name: 'GlobalVideoRegistry',
        category: LogCategory.video,
      );
    }
