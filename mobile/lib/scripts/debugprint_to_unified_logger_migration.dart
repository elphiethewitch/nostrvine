...
const Map<String, String> logLevelPatterns = {
  // Error patterns
  '❌|Error|Failed|Exception|Crash': 'Log.error',

  // Warning patterns
  '⚠️|Warning|Warn|Deprecated|Retry|Skipping|Missing': 'Log.warning',

  // Info patterns (important state changes)
  '✅|Success|Completed|Connected|Initialized|Started|Stopped|Created|Found':
      'Log.info',

  // Debug patterns (detailed operational info)
  '🔍|🔄|📡|🎬|🎥|📥|🎯|Creating|Loading|Processing|Handling|Switching|Checking':
      'Log.debug',

  // Verbose patterns (very detailed tracing)
  '📝|🏷️|🖼️|👤|- Authors:|- Hashtags:|- Since:|- Until:|- Limit:|Detailed|Trace':
      'Log.verbose',
};
...
Future<bool> migrateFile(String filePath) async {
  ...
  var hasDebugPrint = false;
  var hasUnifiedLoggerImport = false;
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (line.contains('debugPrint(')) {
      ...
      if (match != null) {
        ...
        // Create new log call
        final newLogCall =
            "$indentation$logLevel('$cleanMessage', name: '$serviceName', category: $category);";
...
void main() async {
  ...
  print(
      '📊 Migrated $migratedFiles files out of ${dartFiles.length} total Dart files');
  print('');
  print('🔧 Next steps:');
