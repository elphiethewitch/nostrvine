...
void main() {
  ...
  group('ExploreVideoManager Trending Sync Tests', () {
    test(
        'syncs trending videos directly from CurationService without VideoManager filtering',
        () async {
    ...
    test('reflects CurationService changes when listener is triggered',
        () async {
      // Initially no videos
      when(mockCurationService.getVideosForSetType(CurationSetType.trending))
          .thenReturn([]);
      ...
      await exploreVideoManager.refreshCollections();
      expect(exploreVideoManager.getVideosForType(CurationSetType.trending),
          isEmpty);
