  ...
  group('BuildConfiguration', () {
    ...
    test('should provide all flags with defaults', () {
      ...
      for (final flag in FeatureFlag.values) {
        ...
        expect(hasDefault, isTrue, reason: 'Flag ${flag.name} should have a default value');
      }
    });
    ...
    test('should provide environment variable key mapping', () {
      ...
      expect(config.getEnvironmentKey(FeatureFlag.newCameraUI), equals('FF_NEW_CAMERA_UI'));
      expect(config.getEnvironmentKey(FeatureFlag.debugTools), equals('FF_DEBUG_TOOLS'));
      expect(config.getEnvironmentKey(FeatureFlag.enhancedVideoPlayer), equals('FF_ENHANCED_VIDEO_PLAYER'));
    });
  });
}