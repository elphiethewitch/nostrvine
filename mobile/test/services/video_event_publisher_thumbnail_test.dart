...
void main() {
  group('VideoEventPublisher Thumbnail Integration', () {
    ...
    test('publishDirectUpload includes thumbnail in event tags', () async {
      ...
      expect(
          capturedTags.any((tag) =>
              tag.length >= 2 &&
              tag[0] == 'thumb' &&
              tag[1] == 'https://cdn.example.com/thumb123.jpg'),
          isTrue);
      expect(
          capturedTags.any((tag) =>
              tag.length >= 2 &&
              tag[0] == 'url' &&
              tag[1] == 'https://cdn.example.com/video123.mp4'),
          isTrue);
      expect(
          capturedTags.any((tag) =>
              tag.length >= 2 && tag[0] == 'title' && tag[1] == 'Test Video'),
          isTrue);
      expect(
          capturedTags.any((tag) =>
              tag.length >= 2 && tag[0] == 'client' && tag[1] == 'nostrvine'),
          isTrue);
    });
    ...
    test('publishDirectUpload works without thumbnail', () async {
      ...
      final capturedTags = verification.captured[0] as List<List<String>>;
      expect(capturedTags.any((tag) => tag.isNotEmpty && tag[0] == 'thumb'),
          isFalse);
      expect(
          capturedTags.any((tag) =>
              tag.length >= 2 &&
              tag[0] == 'url' &&
              tag[1] == 'https://cdn.example.com/video456.mp4'),
          isTrue);
    });
    ...
    test('creates proper NIP-71 compliant event structure', () async {
      ...
      when(
        ...
        final tags = invocation.namedArguments[const Symbol('tags')]
            as List<List<String>>;
      ...
      expect(
          tags.any((tag) =>
              tag.length >= 2 &&
              tag[0] == 'url' &&
              tag[1] == 'https://cdn.example.com/nip71.mp4'),
          isTrue);
      expect(
          tags.any((tag) =>
              tag.length >= 2 && tag[0] == 'm' && tag[1] == 'video/mp4'),
          isTrue);
      ...
      expect(
          tags.any((tag) =>
              tag.length >= 2 &&
              tag[0] == 'thumb' &&
              tag[1] == 'https://cdn.example.com/nip71-thumb.jpg'),
          isTrue);
      expect(
          tags.any((tag) =>
              tag.length >= 2 && tag[0] == 'title' && tag[1] == 'NIP-71 Test'),
          isTrue);
      expect(
          tags.any((tag) =>
              tag.length >= 2 &&
              tag[0] == 'summary' &&
              tag[1] == 'Testing NIP-71 compliance'),
          isTrue);
      expect(
          tags.any(
              (tag) => tag.length >= 2 && tag[0] == 't' && tag[1] == 'nip71'),
          isTrue);
      expect(
          tags.any(
              (tag) => tag.length >= 2 && tag[0] == 't' && tag[1] == 'nostr'),
          isTrue);
      expect(
          tags.any((tag) =>
              tag.length >= 2 && tag[0] == 'client' && tag[1] == 'nostrvine'),
