...
void main() {
  ...
  group('Upload → Publish E2E Comprehensive Test', () {
    ...
    test('FULL E2E: Upload video → Extract thumbnail → Upload thumbnail → Publish to Nostr',
        () async {
      ...
      expect(upload.status, equals(UploadStatus.readyToPublish),
          reason: 'Upload should be ready to publish after startUpload completes');
      ...
      expect(upload.cdnUrl, startsWith('https://cdn.divine.video/'),
      ...
      if (upload.thumbnailPath != null) {
        expect(upload.thumbnailPath, startsWith('https://cdn.divine.video/'),
            reason: 'If thumbnail URL exists, it should be from Blossom CDN');
        print('✅ Thumbnail URL present: ${upload.thumbnailPath}');
      } else {
        print('ℹ️  Thumbnail URL is null (expected - minimal MP4 has no video frames)');
      }
      ...
      final createEventCall = verify(mockAuthService.createAndSignEvent(
      ...
      expect(eventKind, equals(34236),
          reason: 'Should use NIP-71 addressable video kind');
      expect(eventContent, isNotEmpty,
      ...
      expect(videoUrlComponent, equals('url ${upload.cdnUrl}'),
      ...
      if (thumbnailComponent.isNotEmpty) {
        ...
        expect(thumbnailUrl, equals(upload.thumbnailPath),
        ...
        print('ℹ️  No thumbnail in imeta (expected - video has no extractable frames)');
        expect(upload.thumbnailPath, isNull,
            reason: 'If no thumbnail in imeta, upload.thumbnailPath should also be null');
      }
      ...
      expect(mimeComponent, equals('m video/mp4'),
    ...
    test('E2E: Should handle missing thumbnail gracefully', () async {
      ...
      expect(hasImageComponent, isFalse,
          reason: 'imeta should not have image component when thumbnail upload fails');

      print('✅ Test passed: Video published without thumbnail when thumbnail upload fails');
    });
    ...
    test('E2E: Thumbnail URL preservation across multiple updates', () async {
      ...
      if (originalThumbnailUrl == null) {
        print('ℹ️  Thumbnail URL is null (expected - minimal MP4 has no frames)');
      ...
      expect(updatedUpload!.thumbnailPath, equals(originalThumbnailUrl),
      ...
      expect(publishedUpload!.thumbnailPath, equals(originalThumbnailUrl),
          reason: 'Thumbnail URL should persist after status update');

      print('✅ Test passed: Thumbnail URL persisted through multiple state updates');
    });
  });
}
...
void _configureMockBlossomService(MockBlossomUploadService mock) {
  ...
  // Mock Blossom server
  when(mock.getBlossomServer()).thenAnswer((_) async => 'https://cdn.divine.video');
