...
void main() {
  ...
  setUpAll(() async {
    ...
    final testDir =
        await Directory.systemTemp.createTemp('pipeline_failure_test_');
  ...
  group('Pipeline Failure Recovery Tests', () {
    ...
    test('should handle Cloudinary upload failures gracefully', () async {
      ...
      try {
        ...
        expect(result.finalStatus,
            UploadStatus.failed); // Should be marked as failed
        expect(result.error, isNotNull); // Should capture the error

        // Verify the upload is persisted with failure info
        final persistedUpload =
            stack.uploadManager.getUpload(result.upload!.id);
    ...
    test('should handle backend processing timeouts', () async {
      ...
      final stack = await PipelineTestFactory.createTestStack(
        ...
        config: const PipelineTestConfig(
            scenario: PipelineTestScenario.processingFailure),
      );
      ...
      try {
        final testFile = await PipelineTestFactory.createTestFile(
            tempDir, 'processing_timeout_test.mp4');
        ...
        expect(result.success,
            false); // Should not complete due to no ready events
        ...
        expect(stack.videoEventPublisher.publishingStats['is_polling_active'],
            true);
        expect(stack.videoEventPublisher.publishingStats['total_failed'],
            0); // Not a failure, just waiting
      } finally {
        await stack.dispose();
      }
    });
    ...
    test('should handle Nostr relay failures with retry logic', () async {
      ...
      final stack = await PipelineTestFactory.createTestStack(
        ...
        config: const PipelineTestConfig(
            scenario: PipelineTestScenario.nostrFailure),
      );
    ...
    test('should handle network timeouts gracefully', () async {
      ...
      final stack = await PipelineTestFactory.createTestStack(
        ...
        config: const PipelineTestConfig(
            scenario: PipelineTestScenario.networkTimeout),
      );
    ...
    test('should handle malformed API responses', () async {
      ...
      final stack = await PipelineTestFactory.createTestStack(
        ...
        config: const PipelineTestConfig(
            scenario: PipelineTestScenario.malformedResponse),
      );
    ...
    test('should handle partial Nostr relay success correctly', () async {
      ...
      final stack = await PipelineTestFactory.createTestStack(
        ...
        config: const PipelineTestConfig(
            scenario: PipelineTestScenario.partialSuccess),
      );
      ...
      try {
        final testFile = await PipelineTestFactory.createTestFile(
            tempDir, 'partial_success_test.mp4');
    ...
    test('should recover from service restart scenarios', () async {
      ...
      try {
        ...
        await stack1.uploadManager
            .markUploadReadyToPublish(upload.id, 'restart-cloudinary-123');
        ...
        try {
          ...
          debugPrint(
              '🧪 Service restart recovery test - upload recovered: ${persistedUpload != null}');
          ...
          if (persistedUpload != null) {
            ...
            expect(
                persistedUpload.cloudinaryPublicId, 'restart-cloudinary-123');
          }
        } finally {
